...
TMDB_KEY = "11c4ceadf008a5c81bb84fcd7625b853"
...
def format_movie(m):
    ...
    if m.get("backdrop_path"):
        ...
        m["backdrop_url"] = None
    if m.get("videos"):
        trailers = [v for v in m["videos"].get("results", [])
                    if v["type"] == "Trailer" and v["site"] == "YouTube"]
        m["trailer_key"] = trailers[0]["key"] if trailers else None
    if m.get("credits"):
        directors = [c["name"] for c in m["credits"].get("crew", [])
                     if c["job"] == "Director"]
...
def recommend(title: str, n: int = 10):
    ...
    source_detail = tmdb_get(
        f"/movie/{source['id']}",
        {"append_to_response": "similar,credits,videos"}
    )
    ...
    for m in similar:
        ...
        fm["similarity"] = round(min(99, 80 + m.get("vote_average", 7) * 2), 1)
        movies.append(fm)
